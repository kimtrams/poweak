...
def miner(blockchain):
    while STOP == 0:
        while len(blockchain.blockqueue) >= 1:
            ...
            msg = str(node.server_ip)+","+str(ysy) + "," + cpu + "," + '\n'
            f.write(msg)
            f.close()
            ...
            if block.nonce % 2 == 0:
                ...
                for a in range(len(str(block.nonce))):
                    ...
                    random_nonce = random_nonce // 10
            """
            concent = "update||"+block.conv_block_to_str(0,0)+"||appendix||verification: "+ECC_1.sign_ECDSA_msg(block.get_blockhash(),node.server_port).decode()+"\\miner: "+str(node.server_port)+"\\finish_time: "+str(timestamp)
            """
        ...
        if(type(data) == dict):
            ...
            if data["msg"]["tag"] == "block":
                ...
                if data["msg"]["content"] == "FAKE":
                    ...
                    #print("[double_attack]",ECC_1.validate_signature(ver_block_hash,compar_block_hash,node_port))
                    if ECC_1.validate_signature(ver_block_hash,compar_block_hash,node_port) == "[double attack]":
                        Calculate.calculate_num(True ,True ,False ,False ,False ,node_port)
                    else:
                        if ECC_1.validate_signature(ver_block_hash,compar_block_hash,node_port):
                            print("[ecc_in_block]")
                            if blockchain.chain[-1].info.pos == 1:
                ...
                if data["msg"]["content"] == "FAKE":
                    ...
                    if ECC_1.validate_signature(ver_blockchain_hash,compar_blockchain_hash,node_port) == "[double attack]":
                        Calculate.calculate_num(True ,True ,False ,False ,False ,node_port)
                    else:
                        if ECC_1.validate_signature(ver_blockchain_hash,compar_blockchain_hash,node_port):
...
def new_block():
    ...
    node.broadcast("block||"+block.conv_block_to_str(0,0)+"||appendix||"+ECC_1.sign_ECDSA_msg(block.get_blockhash(),node.server_port).decode())
    ...
    if blockchain.chain[-1].info.pos == 1:
        if turst[0] == node.node_list[node.server_port % 5000]:
            blockchain.append_blockqueue(block)
    else:
        blockchain.append_blockqueue(block)
    
def new_block_double_attack():
    node.double_attack = True
    #info = input("new block:")
    global info ##
    info = info + 1 ##
    info_str = "test {info} port {port}".format(info = info,port = node.server_port) ##7/6
...
def fake_aes_new_block():
    ...
    info_str = "test {info} port {port}".format(info = info,port = node.server_port) ##7/6
    block = Block(info_str,Calculate.get_pos(),node.server_ip+" "+str(node.server_port))
    node.broadcast("block||"+block.conv_block_to_str(0,1)+"||appendix||"+ECC_1.sign_ECDSA_msg(block.get_blockhash(),node.server_port).decode())
    blockchain.append_blockqueue(block)
...
def fake_ecc_new_block():
    ...
    info_str = "test {info} port {port}".format(info = info,port = node.server_port) ##7/6
    block = Block(info_str,Calculate.get_pos(),node.server_ip+" "+str(node.server_port))
    node.broadcast("block||"+block.conv_block_to_str(0,0)+"||appendix||"+ECC_2.sign_ECDSA_msg(block.get_blockhash(),node.server_port).decode())
    blockchain.append_blockqueue(block)
...
if __name__ == "__main__":
    ...
    while STOP == 0:
        ...
        if msg == "new block_no_change":
            for i in range(5):
                block = new_block_no_change()
        if msg == "new block_change":
            for i in range(5):
                block = new_block_change()
        if msg == "stop":
            terminal_stop()
